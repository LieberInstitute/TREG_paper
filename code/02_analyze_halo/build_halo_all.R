...
halo_PM_copies <- halo_out$PM %>%
    ...
    pivot_longer(
        cols = starts_with("n_puncta_"), names_to = "RI_gene",
...
halo_all <- rbind(
    halo_main %>% mutate(analysis = "RNAscope_1"),
...
qc_plot_filtered <- ggplot() +
    stat_summary_hex(
        data = halo_all %>% filter(!region_filter),
        aes(x = XMax, y = YMax, z = nucleus_area),
        fun = mean, bins = 100
    ) +
    geom_rect(
